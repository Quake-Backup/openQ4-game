//----------------------------------------------------------------
// MatchEvidenceFileSystem.cpp
//----------------------------------------------------------------

#include "../../../idlib/precompiled.h"
#pragma hdrstop

#include "MatchEvidenceFileSystem.h"

namespace {

static const char *const MP_MATCH_EVIDENCE_WRITABLE_ROOT = "fs_savepath";

} // namespace

mpMatchEvidenceFileSystemWriter::mpMatchEvidenceFileSystemWriter(
		idFileSystem *backend ) :
	fileSystemBackend( backend ) {
}

int mpMatchEvidenceFileSystemWriter::WriteTemp( const char *temporaryQPath,
		const void *data, int bytes ) {
	if ( fileSystemBackend == 0 || data == 0 || bytes < 1 ||
		 bytes >= MP_MATCH_EVIDENCE_STORAGE_JSON_BYTES ||
		 !MPMatchEvidenceStorageIsTemporaryQPath( temporaryQPath ) ) {
		return -1;
	}

	idFile *file = fileSystemBackend->OpenFileWrite( temporaryQPath,
		MP_MATCH_EVIDENCE_WRITABLE_ROOT );
	if ( file == 0 ) {
		return -1;
	}
...
bool mpMatchEvidenceFileSystemWriter::Promote( const char *temporaryQPath,
		const char *finalQPath ) {
	return fileSystemBackend != 0 &&
		MPMatchEvidenceStorageIsPromotionPair( temporaryQPath, finalQPath ) &&
		fileSystemBackend->PromoteFile( temporaryQPath, finalQPath,
			MP_MATCH_EVIDENCE_WRITABLE_ROOT );
}

bool mpMatchEvidenceFileSystemWriter::RemoveTemp( const char *temporaryQPath ) {
	return fileSystemBackend != 0 &&
		MPMatchEvidenceStorageIsTemporaryQPath( temporaryQPath ) &&
